...
describe('total likes', () => {
  ...
  test('when list has no objects, returns null', () => {
    const result = listHelper.total([])
    expect(result).toBe(0)
  })

  test('when list has only one blog', () => {
    const result = listHelper.total(listWithOneBlog)
    expect(result).toBe(5)
  })

  test('when list has 5 different blogs', () => {
    const result = listHelper.total(blogs)
